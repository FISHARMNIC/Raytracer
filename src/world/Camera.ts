...
export class Camera {
    ...
    private create_outgoing_ray(offset: Vec2): Ray {
        ...
        const direction: NormalizedVec3 = position.sub(this.info.position).normalized().keepalive();
    ...
    public scan(each_ray_do: Camera_Callback) {
        ...
        const renderplane_position: Vec2 = new Vec2(0,0);
    ...
    private compute_setup() {
        ...
        const generate_basis = (): NormalizedBasis => {
            ...
            const t_cross_n: Vec3 = t.cross(this.info.normal).keepalive();
            const u_hat: NormalizedVec3 = t_cross_n.normalized().keepalive();  // -1 is just for +x going right
        ...
        const compute_plane_origin = (): Vec3 => {
            ...
            const plane_origin: Vec3 = this.info.position.add(this.info.normal.to_vec3().scaled(this.info.focal_length)).keepalive();
        ...
        const generate_bounds = (): void => {
            ...
            ).scaled(this.info.focal_length).keepalive();
